...
import { deleteUsers, searchUsers } from "../utils/helper";
...
export default function UsersPanel(props) {
  ...
  useEffect(() => {
    searchUsers(search, setPageNum, setFiltered);
    setCheckedCount(0);
  }, [search, renderFlip])
  ...
  const onDeleteUser = () => {
    if (checkedCount)
      deleteUsers(filtered, onRefresh);
  }
  ...
  return (
    <>
      ...
      >
        ...
        <UserList handleChecked={onChecked} checkedAll={checkedAll} list={filtered} page={page}/>
      </Flex>
      {showModal && <CreateUser onClose={onClose} />}
    </>
  );
}
