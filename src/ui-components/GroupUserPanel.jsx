...
import AddUsersInGroup from "./AddUsersInGroup"
import { deleteUsersInGroup, searchUsersInGroup } from "../utils/helper";

export default function GroupUserPanel(props) {
  ...
  useEffect(() => {
    searchUsersInGroup(groupName, search, setPageNum, setFiltered);
    setCheckedCount(0);
  }, [search, renderFlip])
  ...
  const onDeleteUser = () => {
    if (checkedCount)
    deleteUsersInGroup(groupName, filtered, onRefresh);
  }
  ...
  return (
    <>
      ...
      { showModal && <AddUsersInGroup groupName={groupName} onClose={onClose} /> }
    </>
  );
}
